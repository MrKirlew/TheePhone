...
from services.rag_store import RAGStore, DriveDocsIndex
from services.google_auth import GoogleAuthClient
from agents.agent_factories import build_intent_agent, build_planning_agent, build_reflection_agent
from agents.perception_agent import build_perception_agent
from agents.memory_agent import build_memory_agent
from agents.workspace_agent import build_workspace_agent
from agents.orchestrator_agent import OrchestratorAgent
from agents.tool_wrappers import call_openweather, geocode_address, reverse_geocode, call_google_people_api
...
workspace_agent = build_workspace_agent(settings.gemini_model_default)
...
async def handle_chat(request: web.Request):
    ...
    get_contacts = data.get("get_contacts", False)

    if get_contacts and access_token:
        try:
            people_api_results = await call_google_people_api(access_token)
            tool_results.append(people_api_results)
        except Exception as e:
            logger.error(f"Error calling Google People API: {e}")
            tool_results.append({"error": f"Failed to fetch contacts: {e}"})
...
async def handle_index_doc(request: web.Request):
    ...
    text = DriveDocsIndex.fetch_doc_text(doc_id, credentials=creds)
    chunks = DriveDocsIndex.split_into_chunks(text)
